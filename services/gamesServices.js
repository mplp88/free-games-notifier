  ...
  try {
    ...
    return games.map((game) => {
      const freeGamesPath = game.categories.some((c) => c.path === 'bundles')
        ? 'bundles'
        : 'p';
      const urlDefault = 'https://store.epicgames.com/free-games';
      let slug = '';
      ...
      if (slug === '') {
        slug = game.productSlug != '[]' ? game.productSlug : '';
      }
...
function filterNextGames(data) {
  return data.data.Catalog.searchStore.elements.filter(
    (game) =>
      ...
      game.promotions.upcomingPromotionalOffers[0].promotionalOffers.some(
        (offer) => offer.discountSetting.discountPercentage == 0,
      ),
  );
}
